...
void blink_screen(int b_ms, int t_sec, unsigned char value);
...
int main(void){
	...
	while(1){
		...
		if(dir == 1){	// Ascendente
			if(counter == 60){
				blink_screen(200, 5, counter); // 0.2s = 200ms
				counter = 0;
			}
			counter++; // SW on
		}else{	// Descendente
			if(counter == 0){
				blink_screen(500, 5, counter);
				counter = 59;
			}
			else counter--; // SW off
		}
...
void blink_screen(int b_ms, int t_sec, unsigned char value){
	...
	while(cnt++ < t_sec){
		i = 1;
		while(i++ < (b_ms/10)){ // count for 10ms to 500ms fo example = i < 50
			send2displays(toBcd(value));
			delay(10);
		}
		LATB = (LATB & 0x80FF);     // Limpar os valores do ecra
		delay(500);
	}
}
